		...
		<ModalComponent isVisible={isVisible} toggleModal={toggleModal}>
			{tokenList?.length > 0 ? (
				<div>
					...
					{/* <ModalContent>
						<p className='modal-text-sm'>
							The DApp will take 10 tokens from your wallet to perform this operation!
						</p>
						<TokensList
							tokenList={tokenList}
							isLoading={isLoading}
					...
					<ButtonModal onClick={() => onClick(token)} disabled={isLoading} success>
						Vote
					</ButtonModal>
				</div>
			) : (
