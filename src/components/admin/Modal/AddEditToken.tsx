...
const AddEditToken = ({ isVisible, toggleModal, data, getData }: PropsType) => {
	...
	const [isCreate, setIsCreate] = useState<boolean>(
		data?.can_create_campaign === TOKEN_STATUS.ACTIVE
	)
		...
		<ModalComponent
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				...
				<FormItem>
					...
					<Input type='text' {...register('name')} placeholder='Enter new token name...' />
					<ErrorText>{errors?.name?.message}</ErrorText>
				</FormItem>
				<FormItem>
					<Label required>Address:</Label>
					<Input type='text' {...register('address')} placeholder='Enter token address...' />
					<ErrorText>{errors?.address?.message}</ErrorText>
				</FormItem>

				<FormItem>
					<Label>Image Link:</Label>
					<Input type='text' {...register('icon')} placeholder='Enter image link...' />
				</FormItem>
