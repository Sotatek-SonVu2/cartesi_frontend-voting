...
export const abiConfig = (AbiFile: string) => {
	try {
		...
		if (!network) {
			createNotifications(
				NOTI_TYPE.DANGER,
				`Your app network (${networks[CHAIN_ID]?.name.toUpperCase()}) does not match the ${networks[
					chainId
				]?.name.toUpperCase()} network on Metamask. ${NETWORK_ERROR_MESSAGE}`
			)
			return // undefined
		}
...
export const tokenConfig = (token: string) => {
	try {
		...
		if (network.name === 'localhost') {
			tokenContract =
				token === CARTESI_TOKEN
					? require(`../contracts/localhost/CartesiToken.json`)
					: require(`../contracts/localhost/OtherTokens.json`)
		} else {
			tokenContract =
				token === CARTESI_TOKEN
