    ...
    const tokenList = useMemo(() => {
        ...
        switch (listType) {
            ...
            case GET_CAN_VOTE_ACTIVE:
                data = tokenListing.filter((token: tokenType) => token.can_vote === TOKEN_STATUS.ACTIVE && token.status === TOKEN_STATUS.ACTIVE)
                break;
            case GET_CAN_VOTE:
            ...
            case GET_CAN_CREATE_ACTIVE:
                tokenListing.forEach((token: tokenType) => {
                    ...
                    deposit_info.forEach((deposit: DepositInfoType) => {
                        ...
                        if (token.status === TOKEN_STATUS.ACTIVE && token.can_create_campaign === TOKEN_STATUS.ACTIVE
                            && deposit.contract_address === token.address.toLowerCase() && amount > 0
                        ) {
                            obj = {
                                ...token,
                                ...deposit
            ...
            case GET_ACTIVE_HAS_TOKEN:
                tokenListing.forEach((token: tokenType) => {
                    ...
                    deposit_info.forEach((deposit: DepositInfoType) => {
                        const amount = deposit.amount - deposit.used_amount - deposit.withdrawn_amount
                        if (token.status === TOKEN_STATUS.ACTIVE && deposit.contract_address === token.address.toLowerCase() && amount > 0) {
                            obj = {
                                ...token,
                                ...deposit
                            }
