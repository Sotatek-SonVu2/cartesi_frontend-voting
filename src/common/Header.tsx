		...
		<Content>
			...
			<Menu>
				...
				<MenuList>
					...
					<MenuTitle
						onClick={() => navigate(ROUTER_PATH.WITHDRAW, { replace: true })}
						className='withdraw-step'>
						Withdraw
					</MenuTitle>
					<MenuTitle
					...
					{pathname !== ROUTER_PATH.ADMIN && (
						<>
							<MenuTitle
								onClick={() => navigate(ROUTER_PATH.TOKENS, { replace: true })}
								className='tokens-step'>
								Tokens
							</MenuTitle>
				...
				<img
					...
					alt='logoutIcon'
					width={20}
					className='Icon logout-step'
					onClick={handleLogout}
				/>
			</Menu>

			{isVisible && <DepositModal isVisible={isVisible} toggleModal={toggleModal} />}
		</Content>
	)
}
