...
const isPartialVoucher = (n: PartialVoucher | null): n is PartialVoucher => n !== null
 ...
 */
export const getVouchers = async (url: string, inputKeys: InputKeys): Promise<PartialVoucher[]> => {
	...
	if (inputKeys.epoch_index !== undefined && inputKeys.input_index !== undefined) {
		...
		if (data?.epoch?.input?.vouchers) {
			return data.epoch.input.vouchers.nodes.filter<PartialVoucher>(isPartialVoucher)
		} else {
			return []
		}
		...
		if (data?.epoch?.inputs) {
			...
			for (let input of inputs) {
				ret = ret.concat(input.vouchers.nodes.filter<PartialVoucher>(isPartialVoucher))
			}
			return ret
		} else {
			return []
		...
		const { data, error } = await client.query(VouchersDocument, {}).toPromise()
		if (data?.vouchers) {
...
export const getVoucher = async (url: string, id: string): Promise<Voucher> => {
	...
	const { data, error } = await client.query(VoucherDocument, { id }).toPromise()
