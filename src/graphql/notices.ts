...
const isPartialNotice = (n: PartialNotice | null): n is PartialNotice => n !== null
 ...
 */
export const getNotices = async (url: string, inputKeys: InputKeys): Promise<PartialNotice[]> => {
...
export const getNotice = async (url: string, id: string): Promise<Notice> => {
	...
	const { data, error } = await client.query(NoticeDocument, { id }).toPromise()
